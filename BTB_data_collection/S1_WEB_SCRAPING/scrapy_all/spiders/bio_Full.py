...
#IMPORTANT : remember to set the minimum and max_year (they specify the time range and are inclusive)
...
class JournalSpider(scrapy.Spider):
    name="bio_spiderF"
    start_urls = [
        'https://academic.oup.com/bioinformatics/issue',
    ]
        ...
        for year in dict_dates['years']:
            ...
            if int(year) >= minimum_year :
                if int(year) <= max_year:
                    ...
                    yield scrapy.Request(next_year, callback=self.parse_the_year, meta={'splash' : {'endpoint' : 'render.html', 'args' : { 'wait' : 6.0 }}})
    ...
    def parse_the_year(self, response):
        ...
        for issue in issues:
            ...
            yield scrapy.Request(next_issue, callback=self.parse_the_issue, meta={'splash' : {'endpoint' : 'render.html', 'args' : { 'wait' : 6.0 }}})